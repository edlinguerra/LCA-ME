...
# Distribución t-student: Una muestra
...
1. Importa los datos del archivo **datos1.csv** al ambiente R con el nombre **datos** y verifica sus características y estructura. Asegúrate que el archivo esté en el ambiente de trabajo.

```{r,  include =  FALSE}
datos <- read_csv("datos1.csv")
```
...
```{r, echo =  FALSE}
hist(datos$DBO, main = "histograma de la DBO", xlab = paste("DBO (", expression(O[2]), "ml/l/d)"), ylab =" Frecuencia")
abline(v=mean(datos$DBO))
...
sd(datos$DBO)/sqrt(length(datos$DBO))
...
```{r, include =  FALSE}
t.test(datos$DBO, alternative = "greater", mu = 30)
...
# Distribución t-student: Dos muestras
...
```{r, echo =  FALSE}
hist(datos$DBO, main = "histograma de la DBO", xlab = paste("DBO (", expression(O[2]), "ml/l/d)"), ylab =" Frecuencia")
abline(v = mean(datos$DBO[1:25]), col = "blue")
abline(v = mean(datos$DBO[26:50]),  col = "red")

boxplot(DBO~Momento, data = datos)
```  
...
```{r, include =  FALSE}
t.test(DBO~Momento, data = datos, alternative = "l")
...
```{r, include =  FALSE}
t.test(DBO~Momento, data = datos, alternative = "l", var.equal = TRUE)
