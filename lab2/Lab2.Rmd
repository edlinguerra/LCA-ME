...
1. Aceptar la invitación a *Rstudio Cloud* que les envié por correo.
2. Luego se registrarán con el usuario y contraseña que quieran (nota: pueden usar su cuenta gmail para hacerlo automático).
3. Inmediatamente visitarán el repositorio de actividades en **R** de esta asignatura en [GitHub](https://github.com/edlinguerra/LCA-ME). Solo hagan click al enlace.
4. En este punto vamos a clonar el repositorio con HTTPS, para ello copien desde el botón verde **Clone or download**, seleccionando la dirección `https` allí señalada.
5. Entrarán a su sesión de 
*Rstudio Cloud* y crearán un proyecto nuevo desde un repositorio (**New Project from Git Repo**). Peguen la dirección y listo. Se iniciará una sesión de **RStudio** con los archivos necesarios.
6. Abrirán los archivos Lab2.Rmd, lab2_e1.R y lab2_e2.R en las carpeta **lab2**
7. Desde Lab2.Rmd active *Preview in Viewer Pane* y aplique *Knit*. Puede ahora visualizar la guía desde R-Studio sin necesidad de cambiar de ventanas en su pantalla
8. Lea los pasos de las actividades desde el Viewer de R-Studio y responda y ejecute los comandos desde el archivo de comandos lab2_e1.R para el ejercicio 1 y lab2_e2.R para el ejercicio 2.

# Distribución t-student: Ejericio 1 - Una muestra

Caso hipotético. Se requiere evaluar si una planta procesadora de celulosa está respetando los límites máximos permisibles  de  contaminantes en las descargas de aguas residuales vertidas al río. La normativa ambiental Mexicana especifica varias variables y límites máximo permisible para cada una, entre las que destaca la Demanda Bioquímica de Oxígeno (DBO), cuyo promedio máximo permitido es de 30 ml O<sub>2</sub>/l por día. La normativa indica que el promedio debe ser estimado de una muestra considerando 50 mediciones aleatorias en un lapso de tres meses de completa operatividad de la planta. 
...
2. Calcula la media (con la función `mean`) y desviación estándar (con la función `sd`) de la DBO en la muestra y obtén un histograma (con la función `hist`) para ver su distribución. ¿Te parece que tienen una distribución normal? Confirma que los resultados que obtienes y el gráfico queden así así:

```{r, echo =  FALSE}
hist(datos1$DBO, main = "histograma de la DBO", xlab = paste("DBO (", expression(O[2]), "ml/l/d)"), ylab =" Frecuencia")
...
4. Describe las características de ambos momentos en términos de sus medias, dispersiones y la forma aproximada de sus distribuciones. Ayudate calculando estos estadísticos con las funciones `summary`, `mean`, y `sd`. ¿Cuáles de estas características son parecidas y cuáles diferentes entre las dos muestras? ¿cuál es el error estándar de la media de cada momento?¿qué significa esto respecto a cada momento?
```{r,  include =  FALSE}
...
Por ahora es suficiente. Salve el proyecto con el nombre "laboratorio 2".